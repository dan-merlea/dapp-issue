...
export const getSpritesLayer = (tilesSet: ITextureSets, layers: Tiled.ISpriteLayer[]): Container[] => {
    ...
    layers.forEach((layer, index) => {
        ...
        if (BRING_FORWARD_LAYERS.includes(layer.name)) {
            frontContainer.addChild(layerContainer);
        } else {
            backContainer.addChild(layerContainer);
        }
    });

    return [backContainer, frontContainer];
};
